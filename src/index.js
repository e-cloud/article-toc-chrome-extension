...
function buildAll($headers) {
  ...
  const root = new TreeRoot('test')
  extractTree(root, 0, 0, [-1], $headers)
 ...
 * @param lastLoc
 * @param parentLocStack
 * @param headerList {Element[]}
 */
function extractTree(parentNode, currentLoc, lastLoc, parentLocStack, headerList) {
  ...
  if (currentLoc === 0
    ...
    node.setParent(parentNode).setRoot(parentNode.root).setData({
      header: headerList[currentLoc]
    })
    parentNode.addChild(node)
    extractTree(parentNode, currentLoc + 1, currentLoc, parentLocStack, headerList)
  }
  ...
  // when current is higher than last, create a sub-container in last, then append the current
    ...
    buildToC(parent, currentLoc, currentLoc, parentLocStack, headerList)
  }
  ...
  // rollback the lastLevel loc track point
    ...
    if (parentNode.getParent() === parentNode.root) {
      ...
      buildToC(parentNode.root, currentLoc, currentLoc, parentLocStack, headerList)
    }
    // still inside the toc child scope, find its grandparent container
    else {
      buildToC(parentNode.root, currentLoc, newLastLoc, parentLocStack, headerList)
    }
  }
}
 ...
 * @param lastloc the last location of header list,
 *        may change in traversal due to jumping from high to low
