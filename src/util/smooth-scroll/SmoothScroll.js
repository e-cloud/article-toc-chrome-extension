...
// Other Variables
  ...
  function keydown(event) {
    ...
    let shift, x = 0, y = 0;
    const overflowing = overflowingAncestor(activeElement);
  ...
  let cache = {}; // cleared out after a scrolling session
  let clearCacheTimer;
...
// auto    |   no   |   YES   |   YES  |   YES  |
  ...
  function isContentOverflowing({ clientHeight, scrollHeight }) {
    return clientHeight + 10 < scrollHeight;
  }
...
// for all other elements
  ...
  function isNodeName({ nodeName }, tag) {
    return (nodeName || '').toLowerCase() === tag.toLowerCase();
  }
  ...
  function isInsideYoutubeVideo({ target }) {
    let elem = target;
    let isControl = false;
  ...
  function pulse_(x) {
    ...
    if (x < 1) { // acceleartion
      ...
      // tail the previous animation ended here:
      start = Math.exp(-1);
      // simple viscous drag
      x -= 1;
      expx = 1 - Math.exp(-x);
